    ...
    processDefinitions() {
        ...
        if (this.data.relics && this.data.relics.Relics) {
            this.data.relics.Relics.forEach(r => {
                ...
                if (Array.isArray(r.intrinsicAffixes) && r.intrinsicAffixes.length > 0) {
                    r.intrinsicAffixes = r.intrinsicAffixes.map(partialAffix => {
                        if (!partialAffix || !partialAffix.id) return null;

                        const fullAffixDef = this.definitions.affixes[partialAffix.id] || (this.definitions.affixes[partialAffix.id] = partialAffix);

                        fullAffixDef.eAffixRarity = "Unique";
                        delete fullAffixDef.tierRollRanges;

                        r.implicitAffixPool = r.implicitAffixPool || [];
                        if (!r.implicitAffixPool.some(p => p.value && p.value.id === partialAffix.id)) {
                            r.implicitAffixPool.push({ weight: 1.0, value: fullAffixDef });
                        }

                        (r.canDrop !== false ? droppableAffixIds : this.nonDroppableAffixIds).add(partialAffix.id);
                        return fullAffixDef;
                    }).filter(Boolean);
                }
            });
                    ...
                    Object.entries(sizeConfig.implicitAffixPool).forEach(([poolName, entries]) => {
                        ...
                        if (catId !== undefined && Array.isArray(entries)) {
                            entries.forEach(entry => {
                                let affixId = null;
                                if (entry.value) {
                                    if (typeof entry.value === 'object' && entry.value.id !== undefined) {
                                        affixId = entry.value.id;
                                    } else if (typeof entry.value === 'string') {
                                        const match = entry.value.match(/^\[(\d+)\]/);
                                        if (match) {
                                            affixId = parseInt(match[1], 10);
                                        }
                                    }
                                }
                                if (affixId !== null) {
